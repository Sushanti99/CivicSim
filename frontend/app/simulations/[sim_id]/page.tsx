...
import { AgentRecord, AnswerProb, SimulationDetail } from "@/lib/api";
...
type BreakdownRow = { group: string; counts: Record<string, number>; total: number };
...
function computeBreakdown(
  ...
  return groupOrder
    ...
    .map((g) => ({
      group: g,
      counts: tally[g],
      total: Object.values(tally[g]).reduce((s, v) => s + v, 0),
    }));
}
...
function generateInsights(
  ...
  const pct = (row: BreakdownRow, ans: string) =>
    row.total > 0 ? Math.round(((row.counts[ans] ?? 0) / row.total) * 100) : 0;

  const insights: string[] = [];

  if (ageRows.length >= 2) {
    const sorted = [...ageRows].sort((a, b) => pct(b, topAnswer) - pct(a, topAnswer));
    const diff = pct(sorted[0], topAnswer) - pct(sorted[sorted.length - 1], topAnswer);
    if (diff >= 15) {
      insights.push(`Generational divide: ${sorted[0].group} show ${diff}pp higher "${topAnswer}" than ${sorted[sorted.length - 1].group}.`);
    } else {
  ...
  if (raceRows.length >= 2) {
    const sorted = [...raceRows].sort((a, b) => pct(b, topAnswer) - pct(a, topAnswer));
    insights.push(
      `Highest "${topAnswer}" among ${sorted[0].group} respondents (${pct(sorted[0], topAnswer)}%), lowest among ${sorted[sorted.length - 1].group} (${pct(sorted[sorted.length - 1], topAnswer)}%).`,
    );
  }
  ...
  if (incomePresent.length >= 2) {
    const vals = incomePresent.map((g) => {
      ...
      return row ? pct(row, topAnswer) : null;
    }).filter((v): v is number => v !== null);
    const trend = vals[vals.length - 1] - vals[0];
    if (Math.abs(trend) >= 10) {
      insights.push(`Income ${trend > 0 ? "positively" : "inversely"} related to "${topAnswer}" (${trend > 0 ? "+" : ""}${trend}pp low→high income).`);
    }
  }

  const backedOff = agents.filter((a) => a.backoff_steps.length > 0).length;
  if (backedOff > agents.length * 0.5) {
    insights.push(`${backedOff}/${agents.length} agents required prior backoff — cell-level ATP data sparse for this configuration.`);
  }
  ...
  "3__region_west__economy": {
    ...
    agents: [
      ...
      mockAgent(6, "65 years and over", "White alone", "$150,000 or more", "Retired executive", "Strongly oppose",
        "Mandating wages distorts labor markets. Businesses should be free to set wages based on productivity. This will cause unemployment among the very workers it aims to help.", [
...
export function generateStaticParams() {
  return Object.keys(MOCK_DETAILS).map((sim_id) => ({ sim_id }));
}
...
function BreakdownBars({ rows, answers }: { rows: BreakdownRow[]; answers: string[] }) {
  if (rows.length === 0) return null;
  return (
    <div className="space-y-4">
      {rows.map((row) => (
        <div key={row.group}>
          <div className="mb-1.5 text-xs font-semibold text-[color:var(--color-text-dim)]">
            {row.group} <span className="font-normal text-[color:var(--color-text-faint)]">(n={row.total})</span>
          </div>
          <div className="flex h-6 w-full overflow-hidden rounded-full">
            {answers.map((ans, i) => {
              const w = row.total > 0 ? Math.round(((row.counts[ans] ?? 0) / row.total) * 100) : 0;
              if (w === 0) return null;
              return (
                <div
                  key={ans}
                  title={`${ans}: ${w}%`}
                  style={{ width: `${w}%`, background: answerColor(i) }}
                />
              );
            })}
          </div>
          <div className="mt-1 flex flex-wrap gap-x-3 gap-y-0.5">
            {answers.map((ans, i) => {
              ...
              if (w === 0) return null;
              return (
                <span key={ans} className="text-[10px] text-[color:var(--color-text-faint)]">
                  <span style={{ color: answerColor(i) }}>■</span> {ans} {w}%
                </span>
              );
            })}
          </div>
...
export default async function SimulationDetailPage({
  ...
  const data: SimulationDetail | null = MOCK_DETAILS[sim_id] ?? null;
  ...
  const ageRows = computeBreakdown(agents, binAge, "age", ["Under 18", "18–29", "30–49", "50–64", "65+"]);
  const raceRows = computeBreakdown(agents, binRace, "race", ["White", "Black", "Hispanic", "Asian", "Other"]);
  const incomeRows = computeBreakdown(agents, binIncome, "income", ["<$30k", "$30–60k", "$60–100k", ">$100k"]);
    ...
    <Shell>
      ...
      {sortedDist.filter((d) => d.prob > 0).length > 0 && (
        <div className={`mb-8 grid gap-4 ${sortedDist.filter((d) => d.prob > 0).length >= 3 ? "sm:grid-cols-3" : "sm:grid-cols-2"}`}>
          {sortedDist.filter((d) => d.prob > 0).slice(0, 4).map((d, i) => (
            <StatTile key={d.answer_label} label={d.answer_label} value={`${Math.round(d.prob * 100)}%`} color={answerColor(i)} sub="of simulated population" />
          ))}
        </div>
      )}
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-2">
        {ageRows.length > 0 && (
          <SectionCard title="Support by Age Group">
            <BreakdownBars rows={ageRows} answers={answers} />
          </SectionCard>
        )}
        {raceRows.length > 0 && (
          <SectionCard title="Support by Race / Ethnicity">
            <BreakdownBars rows={raceRows} answers={answers} />
          </SectionCard>
        )}
        {incomeRows.length > 0 && (
          <SectionCard title="Support by Income Level">
            <BreakdownBars rows={incomeRows} answers={answers} />
          </SectionCard>
        )}
              ...
              <div key={d.answer_label}>
                ...
                <div className="h-3 w-full overflow-hidden rounded-full bg-[color:var(--color-fill-track)]">
                  <div
                    ...
                    style={{ width: `${Math.round(d.prob * 100)}%`, background: answerColor(i) }}
                  />
                </div>
              ...
              <tbody className="divide-y divide-[color:var(--color-border)]">
                {agents.map((a, i) => (
                  <tr key={a.agent_id} className="align-top">
                    ...
                    <td className="py-3 pr-4 text-xs text-[color:var(--color-text-dim)]">{binAge(a.demographics.age ?? "")}</td>
                    <td className="py-3 pr-4 text-xs text-[color:var(--color-text-dim)]">{binRace(a.demographics.race ?? "")}</td>
                    <td className="py-3 pr-4 text-xs text-[color:var(--color-text-dim)]">{binIncome(a.demographics.income ?? "")}</td>
                    ...
                    <td className="py-3 text-xs leading-relaxed text-[color:var(--color-text-dim)]">{a.rationale}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {(data.selected_dims ?? []).map((d) => (
                  <span key={d} className="rounded-full border border-[color:var(--color-cyan)]/30 bg-[color:var(--color-cyan)]/8 px-3 py-1 font-mono text-xs text-[color:var(--color-cyan)]">
                    {d}
                  </span>
                ))}
              </div>
            ...
            <div>
              ...
              <div className="space-y-1.5">
                {([
                  ...
                  ["ATP priors source", "Pew ATP 2021 (synthetic demo)"],
      ...
      <header className="sticky top-0 z-20 border-b border-[color:var(--color-border)] bg-[color:var(--color-bg)]/80 backdrop-blur">
        <div className="mx-auto flex h-16 max-w-[1280px] items-center justify-between px-8">
          ...
          <nav className="flex items-center gap-6 text-sm">
            <Link href="/simulate" className="text-[color:var(--color-text-dim)] hover:text-[color:var(--color-text)]">Simulate</Link>
            <Link href="/simulations" className="text-[color:var(--color-text-dim)] hover:text-[color:var(--color-text)]">Simulations</Link>
          </nav>
        </div>
      </header>
